...
Geo3DBuilder.prototype = {
    ...
    this._labelsBuilder.getLabelPosition = function (dataIndex, positionDesc, distance) {
      ...
      if (coordSys.type === 'geo3D') {
        ...
        var center = self.extrudeY
          ? [(tmp.max[0] + tmp.min[0]) / 2, tmp.max[1] + height, (tmp.max[2] + tmp.min[2]) / 2]
          : [(tmp.max[0] + tmp.min[0]) / 2, (tmp.max[1] + tmp.min[1]) / 2, tmp.max[2] + height]
      }
    }
    this._data = data
  ...
  _updateRegionMesh: function (componentModel, api, start, end) {
    ...
    for (var dataIndex = start; dataIndex < end; dataIndex++) {
      ...
      console.log(borderColorArr)
      var borderStyle = graphicGL.parseColor(itemStyleModel.get('borderStyle'))
      colorArr[3] *= opacity
      ...
      if (hasLine) {
        ...
        this._updateLinesGeometry(linesMesh.geometry, componentModel, dataIndex, regionHeight, lineWidth, componentModel.coordinateSystem.transform, borderStyle)
      }
  ...
  _updatePolygonGeometry: function (componentModel, geometry, dataIndex, regionHeight, vertexOffset, triangleOffset, color) {
    ...
    function addVertices(polygon, y, insideOffset) {
      ...
      for (var k = 0; k < pointsLen; k += 3) {
        ...
        currentPosition[extrudeCoordIndex] = y // 高度
  ...
  _updateLinesGeometry: function (geometry, componentModel, dataIndex, regionHeight, lineWidth, transform, borderStyle = 'solid') {
    ...
    coords.forEach(function (geo, index) {
      var exterior = geo.exterior
      var interiors = geo.interiors
      geometry.addPolyline(convertToPoints3(exterior), whiteColor, lineWidth, borderStyle)
      for (var i = 0; i < interiors.length; i++) {
