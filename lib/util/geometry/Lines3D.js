...
var LinesGeometry = Geometry.extend(
  ...
  } /** @lends module: echarts-gl/util/geometry/LinesGeometry.prototype */,
  {
     ...
     */
    addPolyline: function (points, color, lineWidth, borderStyle, startOffset, pointsCount) {
      if (!points.length) {
        return
      }
      var is2DArray = typeof points[0] !== 'number'
      ...
      for (var k = startOffset; k < pointsCount; k++) {
        // 实现虚线效果
        // console.log('k', k)
        // // 如果k是2的倍数，就是偶数，他的线的宽度就设置为0
        // const newLineWidth = k % 2 === 0 ? 0 : lineWidth
        ...
        if (is2DArray) {
          ...
          point = point || []
          point[0] = points[k3] // x
          point[1] = points[k3 + 1] // y
          point[2] = points[k3 + 2] // z
          if (notSharingColor) {
            var k4 = k * 4
            ...
            if (k % 2 === 0) {
              pointColor = [0.3, 1, 0.56, 1]
              // pointColor=[1,1,1,1]
              // console.log('2', pointColor)
            } else if (k % 3 === 0) {
              pointColor = [0.1, 0.85, 0.83, 1]
              // console.log('3', pointColor)
            } else {
              pointColor = [0,0,0,0]
            }
          }
